...
function openJournalModal() {
    // Guard against missing elements (e.g. when no journal modal is present)
    if (journalOverlay && journalModal && journalText) {
        journalOverlay.classList.add('active');
        journalModal.classList.add('active');
        journalText.focus();
    } else {
        // If no modal exists, immediately log the mood without a journal entry
        saveMoodWithJournal();
    }
}
...
function saveMoodWithJournal() {
    const moodData = {
        ...
        // If there is no journalText element, store an empty note
        note: journalText ? journalText.value : '',
        timestamp: Date.now(),
        t: Date.now(),
...
if (logMoodBtnEl) logMoodBtnEl.addEventListener('click', openJournalModal);
if (clearLogBtnEl) clearLogBtnEl.addEventListener('click', clearAllLogs);
if (saveJournalBtn) saveJournalBtn.addEventListener('click', saveMoodWithJournal);
...
if (journalOverlay) journalOverlay.addEventListener('click', closeJournalModal);
